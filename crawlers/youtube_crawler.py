...
import json, time, sys, logging, urllib.parse
...
# 로그 설정
logger = logging.getLogger("crawler.youtube")
...
def search_youtube_videos(query, max_results=20):
    ...
    try:
        logger.info(f"YouTube 검색 시작: {query} (최대 {max_results}개)")
        ...
        with yt_dlp.YoutubeDL(YDL_FLAT_OPTS) as ydl:
            ...
            logger.info(f"검색 결과에서 {len(video_ids)}개 영상 ID 찾음")
            return video_ids
...
def list_channel_video_ids(channel_url: str, limit: int) -> list[str]:
    ...
    try:
        with yt_dlp.YoutubeDL(YDL_FLAT_OPTS) as ydl:
            ...
            logger.info(f"채널에서 {len(video_ids)}개 영상 ID 찾음")
            return video_ids
    except Exception as e:
        logger.error(f"채널 영상 목록 가져오기 오류: {e}")
        return []
...
def fetch_video_meta(video_id: str) -> dict:
    ...
    try:
        ...
        logger.error(f"메타데이터 가져오기 실패({video_id}): {e}")
        return {
...
def fetch_caption_text(video_id: str) -> str:
    ...
    try:
        ...
        logger.info(f"자막 없음 ({video_id}): {e}")
        return ""
    except Exception as e:
        logger.error(f"자막 파싱 실패 ({video_id}): {e}")
        return ""
...
def crawl_youtube_search(search_query: str, max_videos: int, visited_urls=None) -> list[dict]:
    ...
    if visited_urls is None:
        visited_urls = set()
        logger.info(f"새로운 검색 크롤링 세션 시작: {search_query}")
    else:
        logger.info(f"증분 검색 크롤링 시작: {search_query} (기존 URL {len(visited_urls)}개)")
...
def crawl_youtube_channel(channel_url: str, max_videos: int, visited_urls=None) -> list[dict]:
    ...
    if visited_urls is None:
        visited_urls = set()
        logger.info(f"새로운 채널 크롤링 세션 시작: {channel_url}")
    else:
        logger.info(f"증분 채널 크롤링 시작: {channel_url} (기존 URL {len(visited_urls)}개)")
...
def process_video_ids(video_ids, visited_urls):
    ...
    for vid in video_ids:
        ...
        if url in visited_urls:
            logger.debug(f"이미 방문한 URL: {url}")
            continue
            
        # 방문 기록에 추가
        visited_urls.add(url)
        logger.info(f"영상 처리 중: {url}")
    ...
    logger.info(f"YouTube 크롤링 완료: {len(results)}개 영상 (소요 시간: {elapsed_time:.1f}초)")
    ...
    logger.info(f"[OK] 결과 저장 완료: {SAVE_PATH}")
    return results
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"youtube_crawler_{datetime.now():%Y%m%d}.log", encoding='utf-8')
        ]
    )
