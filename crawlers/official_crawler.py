...
# 로깅 설정
logger = logging.getLogger("crawler.official")
...
# 📌 4. 전체 크롤링 실행
def crawl_df(max_pages=2, max_depth=2, visited_urls=None):
    """공식 사이트 전체 크롤링 실행"""
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"official_crawler_{datetime.now():%Y%m%d}.log")
        ]
    )
    
    # 테스트 실행
    crawl_df(max_pages=2, max_depth=2)